        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-xs font-mono font-medium tracking-widest uppercase text-sky">Gallery</span>
            </div>
            <h1 className="text-3xl md:text-5xl font-semibold tracking-tight mb-4">Visitor Photo Gallery</h1>
            <p className="text-sub max-w-xl">Stunning moments from Uttarshall Valley captured by our visitors.</p>
          </div>
          ...
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setCategory(cat)}
              className={`px-4 py-2 text-xs font-semibold uppercase tracking-wide border rounded transition-all ${
                category === cat ? 'border-sky text-sky' : 'border-[rgba(56,189,248,0.15)] text-sub hover:border-sky hover:text-sky'
              }`}>
              ...
              <div key={photo.id} onClick={() => setLightbox(i)}
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity"
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <p className="text-xs text-sky uppercase tracking-wide mb-1">{photo.location}</p>
                    <p className="text-sm">{photo.caption}</p>
                    <p className="text-xs text-sub mt-1">by {photo.author_name}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {lightbox !== null && photos[lightbox] && (
        <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center"
          ...
          <button onClick={e => { e.stopPropagation(); prev() }}
            className="absolute left-4 top-1/2 -translate-y-1/2 text-white text-3xl hover:text-sky z-10">‹</button>
          <img src={photos[lightbox].image} alt={photos[lightbox].caption}
            className="max-w-[90vw] max-h-[90vh] object-contain rounded-lg"
            onClick={e => e.stopPropagation()} />
          <button onClick={e => { e.stopPropagation(); next() }}
            className="absolute right-4 top-1/2 -translate-y-1/2 text-white text-3xl hover:text-sky z-10">›</button>
          <button onClick={() => setLightbox(null)}
            className="absolute top-4 right-4 text-white text-3xl hover:text-sky z-10">✕</button>
        </div>
      )}
...
function UploadModal({ onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-[100] flex items-center justify-center p-4">
      <div className="bg-panel border border-[rgba(56,189,248,0.15)] rounded-xl p-8 w-full max-w-lg relative">
        <button onClick={onClose} className="absolute top-4 right-4 text-sub hover:text-white text-xl">✕</button>
        <h3 className="text-2xl font-semibold mb-2">Upload Your Photo</h3>
        <p className="text-sub text-sm mb-6">Photos are reviewed before appearing in the gallery.</p>
        {error && <p className="text-red-400 text-sm mb-4 p-3 bg-red-400/10 rounded">{error}</p>}
        <form onSubmit={submit} className="space-y-4">
          <div className="border-2 border-dashed border-[rgba(56,189,248,0.3)] rounded-lg p-8 text-center hover:border-sky transition-colors cursor-pointer"
            onClick={() => document.getElementById('photo-file').click()}>
            {file ? <p className="text-sm text-sky">{file.name}</p> : (
              <>
                <p className="text-4xl mb-2">📤</p>
                <p className="text-sm text-sub">Click to browse your photo</p>
                <p className="text-xs text-sub mt-2">JPG, PNG up to 10MB</p>
              </>
            )}
            <input id="photo-file" type="file" accept="image/*" className="hidden" onChange={e => setFile(e.target.files[0])} />
          </div>
          <div>
            <label className="text-xs text-sub uppercase tracking-wide mb-2 block">Caption</label>
          ...
          <div>
            <label className="text-xs text-sub uppercase tracking-wide mb-2 block">Location</label>
          ...
          <div>
            <label className="text-xs text-sub uppercase tracking-wide mb-2 block">Category</label>
            <select value={form.category} onChange={e => setForm({...form, category: e.target.value})}
              className="w-full bg-void border border-[rgba(56,189,248,0.15)] rounded px-4 py-3 text-sm focus:border-sky focus:outline-none appearance-none">
