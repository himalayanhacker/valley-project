    ...
    <div className="pt-16 min-h-screen bg-panel">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="mb-12">
          ...
          <p className="text-sub max-w-xl">Curated Himalayan experiences in Mandi district. All prices in INR per person.</p>
        </div>
          ...
          {packages.map((pkg, i) => (
            <div key={pkg.id} className={`border ${pkg.featured ? 'border-sky' : 'border-[rgba(56,189,248,0.15)]'} rounded-xl overflow-hidden card-hover relative`}>
              ...
              <div className="p-6">
                ...
                <p className="text-sm text-sub mb-4">{pkg.duration_days} Days / {pkg.duration_days - 1} Nights · {pkg.accommodation}</p>
                <ul className="space-y-2 mb-6">
                  {pkg.highlights?.map((h) => (
                    <li key={h} className="flex items-center gap-2 text-sm text-sub">
        ...
        {/* Booking Form */}
        <div className="border border-[rgba(56,189,248,0.15)] rounded-xl p-6 md:p-10 bg-void/50 max-w-2xl mx-auto">
          ...
          {submitted ? (
            <div className="text-center py-8">
              ...
              <p className="text-sub">We'll contact you at {form.email} shortly to confirm your booking.</p>
            </div>
          ) : (
            <form onSubmit={submit} className="space-y-4">
              {error && <p className="text-red-400 text-sm p-3 bg-red-400/10 rounded">{error}</p>}
              <div>
                <label htmlFor="pkg-select" className="text-xs text-sub uppercase tracking-wide mb-2 block">Package</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="check-in" className="text-xs text-sub uppercase tracking-wide mb-2 block">Check-in</label>
                ...
                <div>
                  <label htmlFor="check-out" className="text-xs text-sub uppercase tracking-wide mb-2 block">Check-out</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="adults" className="text-xs text-sub uppercase tracking-wide mb-2 block">Adults</label>
                ...
                <div>
                  <label htmlFor="children" className="text-xs text-sub uppercase tracking-wide mb-2 block">Children</label>
              ...
              <div>
                <label htmlFor="email" className="text-xs text-sub uppercase tracking-wide mb-2 block">Email</label>
                <input id="email" type="email" required value={form.email} onChange={e => setForm({...form, email: e.target.value})}
                  placeholder="your@email.com"
                  className="w-full bg-panel border border-[rgba(56,189,248,0.15)] rounded px-4 py-3 text-sm focus:border-sky focus:outline-none" />
              </div>
              <div>
                <label htmlFor="special-requests" className="text-xs text-sub uppercase tracking-wide mb-2 block">Special Requests</label>
        ...
        <div className="mt-10 p-6 border border-[rgba(56,189,248,0.15)] rounded-lg bg-void/30 max-w-2xl mx-auto">
          <div className="flex items-start gap-4">
            <span className="text-sky text-2xl mt-1">🛡️</span>
            <div>
